...
3. But with OWASP ZAP you can now see that the session-cookie is still in use, and that the session-id is visible without HTTPS, which is bad if someone would have the possibility to eavesdrop. Copy the session-id (see [README.md](README.md))!
